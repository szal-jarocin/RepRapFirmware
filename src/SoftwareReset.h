...
struct SoftwareResetData
{
    ...
    //The last sector size is 32k. IAP requires us to write at least 256bytes and the destination address needs to be on a 256 byte boundary.
...
#elif __LPC17xx__
static_assert(sizeof(SoftwareResetData) <= 256, "Software reset data is larger than page size");
