...
bool Platform::CheckFirmwareUpdatePrerequisites(const StringRef& reply)
{
    ...
    return false;
#else
    FileStore * const firmwareFile = OpenFile(DEFAULT_SYS_DIR, FIRMWARE_FILE, OpenMode::read);
...
# error
    ...
    // Timer Priorities
    NVIC_SetPriority(TIMER1_IRQn, NvicPriorityTimerPWM);
    NVIC_SetPriority(TIMER2_IRQn, NvicPriorityTimerServo);
    NVIC_SetPriority(TIMER3_IRQn, NvicPriorityTimerPWM);
#endif
